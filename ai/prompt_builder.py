...
def build_system_prompt(context: dict) -> str:
    ...
    candidates_block = (
        ...
        "  · 【重要】确定是哪一条候选后，检查它的 sku_summary 里是否包含\"散箱\"字样——这表示原申请里这个商品是散箱（不是整托），"
        "系统没有默认值可用，必须由仓库人员明确说明：这批散箱是从哪个托盘规格（多少箱/托）取出的、取货后该托盘还剩多少箱。"
        "在 reply 中主动询问这两个数字，收集到后填入 extracted_fields 对应的 fulfillment_lines（出库）或 received_lines（入库）里，"
        "格式为 [{{\"sku_code\": \"<该商品编码>\", \"source_boxes_per_pallet\": <取货前箱数/托>, \"resulting_boxes_per_pallet\": <取货后剩余箱数/托>}}]，"
        "在两个数字都收集到之前，绝不能设置 all_fields_collected=true——如果直接确认，系统会报错且该申请将无法再次确认，只能作废重新发起。\n"
        "    示例：唯一候选 REQ-X 的 sku_summary=\"S4 散箱x20\"，用户说\"确认出库\" → 正确输出：extracted_fields 中 reference_serial 填 REQ-X，"
        "all_fields_collected=false，reply 类似\"S4 是散箱出库，请问是从多少箱/托的托盘取货的？取货后这个托盘还剩多少箱？\"。"
        "用户回复\"64箱的托盘，剩44箱\" → extracted_fields 中 fulfillment_lines 填 "
        "[{{\"sku_code\": \"s4\", \"source_boxes_per_pallet\": 64, \"resulting_boxes_per_pallet\": 44}}]，all_fields_collected=true。\n"
