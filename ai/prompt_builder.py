...
def build_system_prompt(context: dict) -> str:
    ...
    candidates_block = (
        ...
        "- storage_buckets：仅供参考，不要自己据此计算 boxes_per_pallet 默认值。outbound 申请中，若某条 sku_lines 未提供 "
        "boxes_per_pallet（客户未说明托盘规格），不要填写、不要猜测、也不要从此列表选择——直接留空，系统会在你回复之后自动、"
        "确定性地解析默认值（或在没有合适库存时直接拒绝该申请），不需要你参与这个判断，通常也不需要为此专门追问客户。"
        "【重要】如果你判断确实有必要就此向客户提问（而不是直接留空交给系统处理），提问时必须引用此列表中该 sku_code+warehouse_code "
        "下真实存在的规格供客户选择，不能是空泛的\"每版上有多少箱？\"——例如\"我们 JFK 仓库现有 S2 37箱/托×1托、64箱/托×10托，"
        "请问您要哪种规格？\"，绝不能凭空猜一个数字反问客户去确认。若该 sku_code+warehouse_code 下此列表中完全没有任何库存，"
        "如实告知客户当前没有库存，而不是询问一个不存在的规格。\n"
