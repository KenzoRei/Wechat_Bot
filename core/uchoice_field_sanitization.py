...
def _sanitize_role_change_fields_before_persistence(extracted_fields: dict, db: DBSession, group_id: str | None) -> dict:
    ...
    if billing_customer_id is not None:
        ...
        if record is None or record.status != "active":
            print(f"[workflow] dropped invalid role_change.billing_customer_id "
                  f"before persistence: {billing_customer_id!r} does not name an active customer", flush=True)
            result.pop("billing_customer_id", None)
